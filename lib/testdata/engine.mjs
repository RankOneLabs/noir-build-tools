    ...
    async execute(yamlPath, cliOptions) {
        ...
        function formatValue(v, fmt) {
            if (v instanceof Uint8Array) {
                ...
                return primitives.toTomlArray(v);
            }
            // Quote bigints and large numbers as strings for Noir TOML parser
            if (typeof v === 'bigint') {
                return `"${v}"`;
            }
            // Quote numbers that might overflow
            if (typeof v === 'number' && (v > 2 ** 53 || v < -(2 ** 53))) {
                return `"${v}"`;
            }
            return v;
        }
        ...
        for (const [key, valExpr] of Object.entries(spec.outputs)) {
            ...
            // Support object syntax for formatting: { val: $v, format: 'hex' }
            if (valExpr && typeof valExpr === 'object' && 'val' in valExpr) {
                val = resolve(valExpr.val, ctx);
                format = valExpr.format;
            } else {
                val = resolve(valExpr, ctx);
            }

            output += `${key} = ${formatValue(val, format)}\n`;
        }
